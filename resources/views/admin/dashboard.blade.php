@extends('admin.layouts.app')
@section('content')
    @auth
        <div class="content-wrapper">
            <div class="row">
                <div class="col-md-12 grid-margin">
                    <div class="row">
                        <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                            <h3 class="font-weight-bold">Welcome {{ Auth::user()->name }}</h3>

                            <h6 class="font-weight-normal mb-0">Selamat datang kembali di halaman dashboard
                                <span class="text-primary">admin!</span>
                            </h6>
                        </div>
                        <div class="col-12 col-xl-4">
                            <div class="justify-content-end d-flex">
                                <div class="dropdown flex-md-grow-1 flex-xl-grow-0">
                                    <span class="btn btn-sm btn-light bg-white">
                                        <div id="dynamicDate"></div>
                                        <script>
                                            // Get the current date
                                            var today = new Date();

                                            // Define months array
                                            var months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

                                            // Format the date
                                            var formattedDate = "Today (" + today.getDate() + " " + months[today.getMonth()] + " " + today.getFullYear() + ")";

                                            // Display the formatted date in the div
                                            document.getElementById('dynamicDate').innerHTML = formattedDate;
                                        </script>
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 grid-margin stretch-card">
                    <div class="card tale-bg">
                        <div class="card-people mt-auto">
                            <img src="{{ asset('backend/images/dashboard/people.svg') }}" alt="people">
                            <div class="weather-info">
                                <div class="d-flex">
                                    <div>
                                        <h2 class="mb-0 font-weight-normal"><i class="icon-sun mr-2"></i><span
                                                id="temp-value">31</span><sup>C</sup></h2>
                                    </div>
                                    <div class="ml-2">
                                        <h4 class="location font-weight-normal" id="city">Jakarta</h4>
                                        <h6 class="font-weight-normal" id="country">Indonesia</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-md-6 grid-margin transparent">
                    <div class="row">
                        <div class="col-md-6 mb-4 stretch-card transparent">
                            <div class="card card-tale">
                                <div class="card-body">
                                    <p class="mb-4">Total Data Penyakit</p>
                                    <p class="fs-30 mb-2">{{ $countDataSickness }}</p>
                                    @if ($latestUpdateSickness)
                                        <p>Update terakhir: {{ $latestUpdateSickness->updated_at->diffForHumans() }}</p>
                                    @else
                                        <p>Belum ada pembaruan</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6 mb-4 stretch-card transparent">
                            <div class="card card-dark-blue">
                                <div class="card-body">
                                    <p class="mb-4">Total Data Gejala</p>
                                    <p class="fs-30 mb-2">{{ $countDataIndication }}</p>
                                    <p>Update terakhir: {{ $latestUpdateIndication->updated_at->diffForHumans() }}</p>
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-4 mb-lg-0 stretch-card transparent">
                            <div class="card card-light-blue">
                                <div class="card-body">
                                    <p class="mb-4">Total Data Edukasi</p>
                                    <p class="fs-30 mb-2">{{ $countDataEducation }}</p>
                                    <p>Update terakhir: {{ $latestUpdateEducation->updated_at->diffForHumans() }}</p>
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6 stretch-card transparent">
                            <div class="card card-light-danger">
                                <div class="card-body">
                                    <p class="mb-4">Total User</p>
                                    <p class="fs-30 mb-2">{{ $countDataUser }}</p>
                                    <p>Update terakhir: {{ $latestUpdateUser->updated_at->diffForHumans() }}</p>
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>

        </div>
    @else
        <script>
            window.location.href = "{{ route('login') }}";
        </script>
    @endauth
@endsection
